...
/// <summary>
/// Tests for the internal VectorRecord class.
...
public class VectorRecordTests
{
    ...
    [Fact]
    public void VectorRecord_HasCorrectAttributes()
    {
    ...
    [Fact]
    public void VectorRecord_EmbeddingAttribute_HasCorrectDimensions()
    {
        // Arrange
        var embeddingProperty = _vectorRecordType.GetProperty("Embedding");
    ...
    [Fact]
    public void VectorRecord_Create_SerializesItemToJson()
    {
        // Arrange
    ...
    [Fact]
    public void VectorRecord_GetItem_DeserializesJsonToItem()
    {
        // Arrange
        var testItem = new TestProduct { Id = 1, Name = "Test Product", Price = 9.99m };
        var embedding = new float[] { 0.1f, 0.2f, 0.3f };
    ...
    [Fact]
    public void VectorRecord_GetItem_ReturnsNullForEmptyJson()
    {
        // This test verifies the null/empty check in GetItem
        // We need to create a record with empty ItemJson

        // Arrange - Create via constructor (internal class, use Activator)
        var record = Activator.CreateInstance(_vectorRecordType, nonPublic: true);
        ...
        // Use property setters (init-only, but we can use reflection)
        var backingFieldId = _vectorRecordType.GetField("<Id>k__BackingField", BindingFlags.NonPublic | BindingFlags.Instance);
        var backingFieldItemJson = _vectorRecordType.GetField("<ItemJson>k__BackingField", BindingFlags.NonPublic | BindingFlags.Instance);
        var backingFieldText = _vectorRecordType.GetField("<Text>k__BackingField", BindingFlags.NonPublic | BindingFlags.Instance);
        var backingFieldEmbedding = _vectorRecordType.GetField("<Embedding>k__BackingField", BindingFlags.NonPublic | BindingFlags.Instance);

        backingFieldId?.SetValue(record, "test-id");
        backingFieldItemJson?.SetValue(record, "");  // Empty JSON
        backingFieldText?.SetValue(record, "test");
        backingFieldEmbedding?.SetValue(record, new ReadOnlyMemory<float>(new float[] { 0.1f }));

        // Act
        var getItemGenericMethod = _getItemMethod.MakeGenericMethod(typeof(TestProduct));
        var result = getItemGenericMethod.Invoke(record, null);
    ...
    [Fact]
    public void VectorRecord_Create_SetsAllProperties()
    {
        // Arrange
